...
// Find the handleCombatantSelect function and replace it
...
// Find the end of the function - look for "  };" after the start
...
const fullEndIdx = startIdx + endIdx + endMarker.length;
...
const newFunction = `  const handleCombatantSelect = async (target) => {
    if (targetingRoll) {
      ...
      // Handle breathing moves and skill tree moves that don't need weapon selection
      const isAbilityTargeting = targetingRoll.isBreathingMove || targetingRoll.isSkillTreeMove;

      if (isAbilityTargeting) {
        // For ability-based targeting, use the pre-rolled dice result and send as a DICE_ROLL
        finishDiceRoll(targetingRoll.diceResult, targetingRoll.input, targetingRoll.playerName, targetingRoll.playerImage, target, null, null, targetingRoll.isBreathingMove ? 'breathing' : 'skilltree');
        setTargetingRoll(null);
        setSelectedWeapon(null);
        return;
      }

      // ENFORCE WEAPON SELECTION for normal combat targeting rolls
      if (!selectedWeapon) {
        if (showToast) {
...
const newContent = content.substring(0, startIdx) + newFunction + content.substring(fullEndIdx);
