import { RANKS, CLASSES_LIST, RESPIRACOES, LINHAGENS, CORES, formatHeight, LINHAGEM_DESCRIPTIONS } from '../constants/gameData';
import { supabase } from '../lib/supabase';
import { TooltipWrapper, CustomSelect } from './UIElements';
...
const BioGrid = memo(({ activeChar, isEditing, setTempChar }) => {
  ...
  return (
    <div className="grid grid-cols-2 gap-x-10 gap-y-8 mt-4">
      {/* IMAGE COLUMN */}
      <div className="row-span-4 flex flex-col items-center justify-center bg-black/40 rounded-xl border border-white/5 relative group overflow-hidden aspect-square w-full">
      ...
      <div className="space-y-1">
        ...
        {isEditing ? (
          <input type="number" value={activeChar?.dollars ?? ""} onChange={(e) => { playSound('stat_point'); updateField('dollars', e.target.value === "" ? "" : Math.max(0, parseInt(e.target.value))); }} className="bg-slate-800 border border-white/10 rounded px-3 py-2 w-full text-sm outline-none" />
      ...
      <SelectField label="Linhagem" field="bloodline" options={LINHAGENS} descriptions={LINHAGEM_DESCRIPTIONS} />
      ...
      <div className="space-y-1">
        ...
        {isEditing ? (
          <input type="number" value={activeChar?.age ?? ""} onChange={(e) => { playSound('stat_point'); updateField('age', e.target.value === "" ? "" : Math.max(0, parseInt(e.target.value))); }} className="bg-slate-800 border border-white/10 rounded px-3 py-2 w-full text-sm outline-none" />
