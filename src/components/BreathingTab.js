...
export default function BreathingTab({ user, character, isMaster, showToast, playSound, onReturn }) {
  ...
  const [optimisticSkills, setOptimisticSkills] = useState(character?.breathing_skills || []);
  const [optimisticPoints, setOptimisticPoints] = useState(character?.breathing_points || 0);

  useEffect(() => {
    if (character) {
      setOptimisticSkills(character.breathing_skills || []);
      setOptimisticPoints(character.breathing_points || 0);
    }
  }, [character?.breathing_skills, character?.breathing_points]);
      ...
      <div className="flex justify-between items-center mb-8 bg-zinc-900/80 p-6 rounded-2xl border border-zinc-800 backdrop-blur-sm z-10">
        ...
        <div className="flex items-center gap-4">
          <div className="bg-cyan-950/30 border border-cyan-500/30 px-6 py-2 rounded-full">
            <span className="text-cyan-400 font-black text-sm">{optimisticPoints} PONTOS DE RESPIRAÇÃO</span>
          </div>
        </div>
      </div>
            ...
            return (
              <div
                ...
                className={`skill-node absolute w-20 h-20 border-2 transition-all duration-300 group bg-black
                  ${isLearned ? 'border-cyan-400 shadow-[0_0_15px_rgba(34,211,238,0.4)] hover:!z-[1000]' : (unlocked ? 'border-cyan-900/50 hover:border-cyan-500 hover:scale-105 hover:!z-[1000]' : 'border-zinc-900 hover:!z-[1000]')}
                `}
              >
                <div className="absolute inset-0 bg-black overflow-hidden pointer-events-none">
                    <img src={iconPath} alt={skill.name} className={`w-full h-full object-cover transition-all duration-300 ${isLearned ? 'opacity-100 brightness-110' : (unlocked ? 'opacity-50 grayscale-[0.75] brightness-75' : 'opacity-50 grayscale-[0.6] brightness-[0.2]')}`} />
                    {!unlocked && !isLearned && (
                ...
                <div className={`absolute bottom-0 left-0 right-0 py-1 border-t border-white/5 z-10 pointer-events-none transition-all ${isLearned ? 'bg-cyan-500/20' : 'bg-black/80'} ${!unlocked && !isLearned ? 'opacity-50' : ''}`}>
                  ...
                  <div className="relative z-[1000] flex flex-col gap-2">
                    <div>
                      <div className="flex justify-between items-center">
                        <p className="text-cyan-400 font-black uppercase text-[11px] leading-tight">
