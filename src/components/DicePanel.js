...
export default function DicePanel({ activeChar, luckPerc }) {
  ...
  return (
    <div className="bg-slate-900/80 p-6 rounded-[30px] border-2 border-cyan-500/30 shadow-[0_0_20px_rgba(6,182,212,0.1)]">
      <h3 className="font-black text-cyan-500 text-[10px] italic mb-4 tracking-widest uppercase">Sistema de Ataque</h3>
      <div className="space-y-3">
        {equippedWeapons.length > 0 ? equippedWeapons.map((w, idx) => (
          <DataRow
            key={idx}
            label={`PAT (${w.name})`}
            value={calculateWeaponPAT(w, activeChar)}
            subtitle={`${w.subtype} | ${w.tier}`}
          />
        )) : (
          <DataRow label="PAT (Arma)" value="Nenhuma Equipada" />
        )}
        <DataRow label="PAT (Desarmado)" value={disarmedPat} subtitle="Soco / Improviso" />
